...
/**
 * Lookup class provides conversion between different encodings and character tables
 * using character name, encoding index, and unicode.
 * In order to add new tables original code should be modified.
 *
 * @author Sami Kama
 * @version $Id: Lookup.java,v 1.1 2005/01/03 21:22:24 duns Exp $
 */
public class Lookup {
     ...
     * This method takes name of the character and
    ...
    /**
     * Gives the requested encoding table
     * Valid table names can be requested with a call to
     * @see #getTableNames().
     *
	...
	public CharTable getTable(String tableName) {
		for (int i=0; i<ntables; i++) {
			...
			if(tblName.equalsIgnoreCase(tableName)) return(tables[i]);
		}
		return(null);
	}
