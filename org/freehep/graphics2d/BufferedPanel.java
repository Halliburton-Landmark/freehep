...
/**
 * This class extends JPanel by adding double buffering. This is
 * intended to be used in situations in which redrawing the contents
 * of the panel is expensive.
 *
 * @author Mark Donszelmann
 * @version $Id: BufferedPanel.java,v 1.7 2005/01/21 19:24:29 duns Exp $
 */
public class BufferedPanel extends JPanel implements java.io.Serializable {
    ...
    /**
     * Triggers a full "user" repaint. If the "system" wants to repaint it will call the
     * paint(Graphics) method directly, rather than scheduling a paint(Graphics)
     * through a repaint().
     */
    public void repaint() {
        super.repaint();
    ...
    /**
     * Triggers a full repaint, since the component is not valid anymore (size change,
     * iconized, ...)
     */
    ...
    /**
     * Returns if true if paintComponent(VectorGraphics) should be called (was triggered
     * by a repaint() or invalidate(), and resets the trigger.
     */
     ...
     * flushing the buffered image to the screen. This method also handles printing
     * and exporting separately.
     *
...
/* FREEHEP-503 Disabled, since in Linux this made the blitting very slow.
        ...
        boolean done = g.drawImage(subimage,clip.x,clip.y,this);
*/

        boolean done = g.drawImage(offScreenImage, 0, 0, this);
//        System.err.println("CopyImage "+done+" took: "+(System.currentTimeMillis()-t0)+" ms.");
    }
     ...
     * <P>
     * NOTE: this method used to be called getGraphics, however, since the JVM paint thread may 
     * call getGraphics from paintImmediately and fails to work with our VectorGraphics context
     * (the gc is not longer attached to the image), we decided to rename the method.
     */
     ...
     * see where the output goes. If painting is done to a display it is done
     * to a buffer which is kept and copied afterwards.
     *
     * Note that the parameter here is of class VectorGraphics rather than Graphics.
     */
     ...
     * True if not Printing and not Exporting.
     */
    ...
    /**
     * Make the buffered image for this panel.  Check to see if the
    ...
    private void makeImage() {
        ...
        if (dim.width>0 && dim.height>0) {
            if (!oldDimension.equals(dim)) {
                ...
                // allocate an Image, which from before JDK 1.4 was always in System Memory,
                // and from JDK 1.4 is a Managed Buffered Image in System memory or VRAM (Windows)
                ...
                // The image created here is a Managed Buffered Image, which can be moved
