 ...
 * @version $Id: LabelPlacer.java,v 1.7 2004/01/16 23:05:10 duns Exp $
 */
...
public class LabelPlacer extends Component {
    ...
    private void addLabels(int n) {
        Dimension d = getSize();
        while (n>0) {
            labels.addElement(new TextLabel("Point "+labels.size(),
                                            new Point(xRandom.nextInt(d.width-2*border), yRandom.nextInt(d.height-2*border)),
                                            locRandom.nextInt(8),
                                            this));
            n--;
        }
    ...
    private void positionLabels(int algorithm) {
        switch (algorithm) {
            case 1: { // Random
                ...
                break; }
            case 2: { // Greedy: Place labels randomly, then for each conflicting label, try the next position
...
class TextLabel {
    ...
    Rectangle getBounds(int position) {
        ...
        return new Rectangle(location.x+(int)(getTx(position)*width), location.y+(int)(getTy(position)*height), width, height);
    }
