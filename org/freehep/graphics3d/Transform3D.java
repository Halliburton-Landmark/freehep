 ...
 * @version $Id: Transform3D.java,v 1.6 2003/03/20 00:20:23 duns Exp $
 */

public class Transform3D implements Serializable {

    protected Stack/*Matrix4*/ transforms;
    protected Matrix4 m;
    ...
    /**
     * returns the world distance of a pixel distance by applying the inverse transform,
     * but without using the translation part
     */
    public Vector3 deltaInverseTransform(double du, double dv, double dw, Vector3 dxyz)
            throws NoninvertibleTransformException {
        Matrix4 i = m.createInverse(null);
        if (dxyz == null) dxyz = new Vector3();
        dxyz.x = i.m00*du + i.m01*dv + i.m02*dw;
        dxyz.y = i.m10*du + i.m11*dv + i.m12*dw;
        dxyz.z = i.m20*du + i.m21*dv + i.m22*dw;
        return dxyz;
    }
     ...
     *
     * The current transform matrix M is pre-multiplied by:
     * <pre>
    ...
    public void translate(double tx, double ty, double tz) {
        try {
            System.out.println("DeltaScreen: "+tx+", "+ty+", "+tz);
            System.out.println("DeltaNorm:   "+(2*tx/500)+", "+(2*ty/500)+", "+tz);
            Vector3 v = deltaInverseTransform(2*tx/500, 2*ty/500, tz, null);
            System.out.println("DeltaUnit:   "+v);
            translateModel(v.x, v.y, v.z);
        } catch (NoninvertibleTransformException e) {
            System.out.println("Cannot invert transform "+m.determinant()+"\n"+m);
        }
    }
...
// FIXME missing methods to retrieve vector and angle.
