 ...
 * @version $Id: TagletManager.java,v 1.5 2004/01/15 19:17:00 duns Exp $
 */
...
import org.freehep.tools.doclet.tags.*;
import java.io.*;
...
public class TagletManager {
    ...
    public void checkTags(Doc doc, Tag[] tags, boolean areInlineTags) {
        ...
        for (int i = 0; i < tags.length; i++) {
            ...
            if (name.equals("")) { //MD
                System.out.println("WARNING '@' without qualifier in "+doc+"!");    //MD
            } else //MD
            if (! (standardTags.contains(name) || customTags.containsKey(name))) {
// added to allow unknown tags to be simple tags                                    //MD
                System.out.println("Added tag '@"+name+"' as SimpleTaglet.");       //MD
                String uCaseName = Character.toUpperCase(name.charAt(0))+name.substring(1); // MD
                customTags.put((taglet = new SimpleTaglet(name, uCaseName+":",           //MD
                                    SimpleTaglet.ALL)).getName(), taglet);          //MD
                resetTags();                                                        //MD
/* startRemoved //MD
   ...
   endRemoved */ //MD
    ...
    private void resetTags() {      //MD
        packageTags =  null;        //MD
        typeTags =  null;           //MD
        fieldTags =  null;          //MD
        constructorTags =  null;    //MD
        methodTags =  null;         //MD
        overviewTags =  null;       //MD
        inlineTags =  null;         //MD
    }                               //MD
