 ...
 * @version $Id: ImageExportFileType.java,v 1.10 2003/12/09 01:15:58 duns Exp $
 */
public class ImageExportFileType extends AbstractExportFileType {
    ...
    public static ImageExportFileType getInstance(String format) {
        try {
            ...
            if (format.equalsIgnoreCase("gif")) return exportFileType("org.freehep.graphicsio.gif.GIFExportFileType");
            if (format.equalsIgnoreCase("png")) return exportFileType("org.freehep.graphicsio.png.PNGExportFileType");
            if (format.equalsIgnoreCase("jpg")) return exportFileType("org.freehep.graphicsio.jpg.JPGExportFileType");
            if (format.equalsIgnoreCase("raw")) return exportFileType("org.freehep.graphicsio.raw.RawExportFileType");
    ...
    private static class SpecialClassLoader extends ClassLoader {
        ...
        public Class findClass(String name) throws ClassNotFoundException {
            try {
                ...
                if (in == null) throw new IllegalArgumentException(templateName+" class does not exist.");
    ...
    public JPanel createOptionPanel(Properties user) {
        ...
        panel.add(TableLayout.FULL,
                    new BackgroundPanel(options, formatKey, ImageGraphics2D.canWriteTransparent(format), null));

        antialias = new OptionCheckBox(options,
                                                formatKey+ImageGraphics2D.ANTIALIAS,
                                                "Antialias");
        panel.add(TableLayout.FULL, antialias);

        antialiasText = new OptionCheckBox(options,
                                                formatKey+ImageGraphics2D.ANTIALIAS_TEXT,
                                                "Antialias Text");
        panel.add(TableLayout.FULL, antialiasText);

        progressive = new OptionCheckBox(options,
                                                formatKey+ImageGraphics2D.PROGRESSIVE,
                                                "Progressive");
        ...
        compress = new OptionCheckBox(options,
                                                formatKey+ImageGraphics2D.COMPRESS,
                                                "Compress");
        if (param.canWriteCompressed()) {
            ...
            param.setCompressionMode(
                options.isProperty(formatKey+ImageGraphics2D.COMPRESS) ?
        ...
        if  (param.canWriteCompressed()
         && (param.getCompressionMode() == ImageWriteParam.MODE_EXPLICIT)) {
            ...
            compressMode = new OptionComboBox(options,
                                                formatKey+ImageGraphics2D.COMPRESS_MODE,
                                                compressionTypes);
...
/* FIXME to be connected to the rest
            String[] compressionDescriptions = param.getCompressionQualityDescriptions();
            JLabel compressDescriptionLabel = new JLabel("Quality Mode");
...
// FIXME check value
            ...
            compressQuality = new OptionTextField(options,
