 ...
 * pattern/shading/function dictionaries to the pdf file when the pageStream is complete.<br>
 * TODO: - reuse pattern dictionaries if possible
 *       - cyclic function not working yet (ps calculation)
 * @author Simon Fischer
 * @version $Id: PDFPaintDelayQueue.java,v 1.7 2005/03/11 05:29:10 duns Exp $
 */
public class PDFPaintDelayQueue {
     ...
     * that is necessary to map the pattern's coordinate space to the
     * default coordinate system of the parent's content stream (in our
     * case the flipping of the page).
     */
    	...
    	while (i.hasNext()) {
    	    ...
    	    if (!e.written) {
        		...
        		if (e.paint instanceof GradientPaint) {
        		    ...
        		    // FIXME, we could write a color here, to keep the file valid.
        	    }
    	    }
    	}
    }
     ...
     *  the resources dicionrary's /Pattern entry. */
    ...
    private void addGradientPaint(Entry e) throws IOException {
    	...
    	shading.entry("Coords", new double[] { p1.getX(), p1.getY(),
    					       p2.getX(), p2.getY() });
    	...
    	if (gp.isCyclic()) {
    // FIXME: cyclic function only eveluated between 0 and 1 for short range, not repetitive.
    ...
    /** Writes a type 4 (PostScript calculator) function that describes a triangle function
     *  to the pdf file. */
    private void addCyclicFunction(String functionRef, double[] c0, double[] c1, double[] dom)
        	throws IOException {
    	//PDFStream function = pdf.openStream(functionRef, new String[] { "Flate", "ASCII85" });
    	...
    	function.println("{");
    	/*
            function.println("2 mod");
    	function.println("1 sub");
    	function.println("abs");
    	function.println("1 exch sub");
    	*/
