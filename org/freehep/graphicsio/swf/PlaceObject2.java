 ...
 * @version $Id: PlaceObject2.java,v 1.4 2003/04/15 18:03:05 duns Exp $
 */
public class PlaceObject2
    ...
    public PlaceObject2(boolean move, int depth, int id, AffineTransform matrix,
                        ColorXform cxform, int ratio, String name, int clipDepth) {
        this();
        this.move = move;
        this.depth = depth;
        character = id;
    ...
    public SWFTag read(int tagID, SWFInputStream swf, int len)
        ...
        if (hasCharacter) tag.character = swf.readUnsignedShort();
        if (hasMatrix) tag.matrix = swf.readMatrix();
        if (hasColorXform) tag.cxform = new ColorXform(swf, true);
        if (hasRatio) tag.ratio = swf.readUnsignedShort();
        if (hasName) tag.name = swf.readString();
        if (hasClipDepth) tag.clipDepth = swf.readUnsignedShort();
        if (hasClipActions) {
    ...
    public void write(int tagID, SWFOutputStream swf) throws IOException {
        ...
        if (character >= 0) swf.writeUnsignedShort(character);
        if (matrix != null) swf.writeMatrix(matrix);
        if (cxform != null) cxform.write(swf, true);
        if (ratio >= 0) swf.writeUnsignedShort(ratio);
        if (name != null) swf.writeString(name);
        if (clipDepth > 0) swf.writeUnsignedShort(clipDepth);
