 ...
 * @version $Id: SWFHeader.java,v 1.6 2005/02/28 06:55:58 duns Exp $
 */
public class SWFHeader implements SWFConstants {
    ...
    public SWFHeader(SWFInputStream swf)
        ...
        // NOTE: page 8 and 13 are in conflict about the HEADER for compressed files.
        // the tag that can be read by Flash is CWS (and not FWC).

        int s = swf.readUnsignedByte();
        if (((int)'F' != s) && ((int) 'C' != s)) throw new IOException();
        if ((int) 'W' != swf.readUnsignedByte()) throw new IOException();
        if ((int) 'S' != swf.readUnsignedByte()) throw new IOException();
        version = swf.readUnsignedByte();
        length = swf.readUnsignedInt();

        compress = (version >= 6) && ((int) 'C' == s);
        if (compress) swf.startDecompressing();
    ...
    public void write(SWFOutputStream swf) throws IOException {
        ...
        if ((version >= 6) && compress) swf.startCompressing();
