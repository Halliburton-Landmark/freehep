 ...
 * @version $Id: DefineText.java,v 1.3 2005/02/28 06:55:58 duns Exp $
 */
public class DefineText
    ...
    protected void read(int tagID, SWFInputStream swf, int len, boolean hasAlpha)
        ...
        Record record = type1 ? (Record)new RecordType1(swf, glyphBits, advanceBits, hasAlpha)
                              : (Record)new RecordType0(swf, glyphBits, advanceBits);
        while (!record.isEndRecord()) {
            text.add(record);
            type1 = swf.readBitFlag();
            record = type1 ? (Record)new RecordType1(swf, glyphBits, advanceBits, hasAlpha)
                           : (Record)new RecordType0(swf, glyphBits, advanceBits);
        }
    }
    ...
    public static class RecordType0 extends Record {
        ...
        RecordType0(SWFInputStream swf, int glyphBits, int advanceBits)
            ...
            if (glyphCount == 0) return;        // end record
        ...
        public void write(SWFOutputStream swf, int glyphBits, int advanceBits, boolean hasAlpha)
            throws IOException {
    ...
    public static class RecordType1 extends Record {
        ...
        RecordType1(SWFInputStream input, int glyphBits, int advanceBits, boolean hasAlpha) throws IOException {
            ...
            if (hasFont) fontID = input.readUnsignedShort();
            if (hasColor) color = input.readColor(hasAlpha);
            if (hasXOffset) xOffset = input.readShort();
            if (hasYOffset) yOffset = input.readShort();
        ...
        public void write(SWFOutputStream swf, int glyphBits, int advanceBits, boolean hasAlpha)
            ...
            if (fontID >= 0) swf.writeUnsignedShort(fontID);
            if (color != null) swf.writeColor(color, hasAlpha);
            if (xOffset != 0) swf.writeShort(xOffset);
            if (yOffset != 0) swf.writeShort(yOffset);
            if (fontID >= 0) swf.writeUnsignedShort(height);
