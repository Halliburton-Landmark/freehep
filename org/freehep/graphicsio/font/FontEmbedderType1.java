 ...
 * @version $Id: FontEmbedderType1.java,v 1.4 2005/01/03 21:22:27 duns Exp $
 */
public class FontEmbedderType1 extends FontEmbedder {
    ...
    /** Defines whether or not encrypted part should be hex encoded (otherwise it is binary). */
    private static final boolean HEX_ENC = true;

    /** Defines whether or not encrypted charstrings should be hex encoded
     *  Ghostview crashes when set to true. The freehep ps interpreter handles it correctly. */
    ...
    protected void writeEncoding(CharTable t) throws IOException {
        ...
        fontFile.println("0 1 255 {1 index exch /.notdef put} for"); // set undefined to .notdef ??
        for (int i = 0; i < 256; i++) {
    ...
    protected void openIncludeFont() throws IOException {
        ...
        fontFile.println("%!FontType1-1.0: " + getFont().getName()); // unknown version number
...
//                     format(new Date()));
    ...
    protected void openGlyphs() throws IOException {
        ...
        if (ENCRYPT) {
            if (HEX_ENC) {
                encrypted =
                    new PrintStream(new EEXECEncryption(new ASCIIHexOutputStream(fontFile),
                                    EEXECEncryption.EEXEC_R));
            } else {
                encrypted =
                    new PrintStream(new EEXECEncryption(fontFile,
                                    EEXECEncryption.EEXEC_R));
            }
        } else {
            encrypted = fontFile;
    ...
    protected void writeGlyph(String characterName, Shape glyph,
        ...
        // FIXME: find out why Acrobat Reader displays some characters displaced when
        //        using the correct sidebearing. A value of 0 looks good
        double sidebearing = glyphMetrics != null ?
            glyphMetrics.getLSB() :
            0;
        ...
        CharstringEncoder charString =
            (HEX_ENC_CHARSTRINGS ?
             new CharstringEncoder(new EEXECEncryption(new ASCIIHexOutputStream(bytes),
    ...
    public int getAsciiLength() { return asciiEnd; }

    /** Returns the length of the encrypted portion of the output. */
    public int getEncryptedLength() { return encEnd - asciiEnd; }
