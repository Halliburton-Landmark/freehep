 ...
 *  Subclasses must implement the <tt>writeGlyph</tt> method which is called
 *  for all defined (up to 256) characters and the notdefined character.
 *  These method calls are bracketed by <tt>openGlyphs()</tt> and <tt>closeGlyph()</tt>.
 *  All invocations of methods that are abstract in this class succeed the method calls
 *  of the superclass <tt>FontIncluder</tt> (especially <tt>closeIncludeFont()</tt>!)
 *  All of these calls are again succeeded by <tt>closeEmbedFont</tt>.
 *  <br>
 ...
 *  @version $Id: FontEmbedder.java,v 1.3 2005/01/03 21:22:27 duns Exp $
 */
public abstract class FontEmbedder extends FontIncluder {
     ...
     *  indicates the undefined character. In this case the value of <tt>unicodeName</tt>
     *  equals the value of <tt>NOTDEF</TT> (=<tt>.notdef</tt>).
    ...
    /** Called after the glyph loop ends. Does nothing by default but can
     *  be implemented. */
    ...
    protected Shape getGlyph(int i) {
       // This one-line implementation produces different results under JDK 1.3 and 1.4
       //return glyphs.getGlyphOutline(i);

       // The substitute code attempts to work around this by using defensive programming
    ...
    private Shape createUndefined() {
        ...
        ud.append(new Rectangle2D.Double(0,               0,
                                         FONT_SIZE,       FONT_SIZE),
                                         false);
        ud.append(new Rectangle2D.Double(FONT_SIZE/20,    FONT_SIZE/20,
                                         18*FONT_SIZE/20, 18*FONT_SIZE/20),
                                         false);
        return ud;
    }
}
