...
public class TestPaint
    ...
    public void paintComponent(Graphics g) {
        ...
        Image limage =
        ImageHandler.getImage("images/BrokenCursor.gif", getClass());
        t.addImage(limage, 0);
        try {
            t.waitForAll();
        ...
        BufferedImage image = new BufferedImage(limage.getWidth(this),
                            limage.getHeight(this),
        ...
        Paint[] paint = new Paint[] {
            ...
            new TexturePaint(image,
               new Rectangle2D.Double(0,0,
                          ...
                          image.getHeight()/2)),
            new TexturePaint(image,
               new Rectangle2D.Double(image.getWidth()/2, image.getHeight()/2,
            ...
            switch(x) {
                case 0:
                    p = new GradientPaint(dw*x,      (int)row*dh,          Color.red,
                            (int)(dw*x+dw*0.9),      (int)(row*dh+0.9*dh), Color.blue);
                    break;
                case 1:
                    p = new GradientPaint(dw*x+dw/4, (int)row*dh+dh/4,     Color.green,
                            (int)(dw*x+dw*0.6),      (int)(row*dh+0.6*dh), new Color(255,0,255));
                    break;
                case 2:
                    p = new GradientPaint(dw*x,      (int)row*dh,          Color.red,
                            (int)(dw*x+dw*0.2),      (int)(row*dh+0.2*dh), Color.yellow,
                            true);
                    break;
            }
            vg.setPaint(p);
        ...
        svg.setPaint(new GradientPaint((int)x1, (int)y1, Color.red,
                      (int)x2, (int)y2, Color.blue));
        svg.fillRect(x1, y1, x2-x1, y2-y1);

        svg.setPaint(new TexturePaint(image,
                      new Rectangle2D.Double(0,0,
                                 image.getWidth(),
                                 image.getHeight())));
