 ...
 * transform, and then also an image tranform with rotation.  The image
 * includes a transparent cell (represented by a cell with 0 alpha).  Note
 * that CGM doesn't fully support alpha, but it supports a transparent
 * cell value, which is used by CGMGraphics2D only when alpha is
 * exactly 0).
 * 
 * @author Ian Graham
 */
...
// GIFGraphics2D graphics = new GIFGraphics2D(out, new Dimension(60, 50)); 
