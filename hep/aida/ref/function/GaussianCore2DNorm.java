...
public class GaussianCore2DNorm extends FunctionCore {
	...
	IModelFunction f1 = (IModelFunction) ff.createFunctionByName("f1","g2");
	IModelFunction f2 = (IModelFunction) ff.createFunctionByName("f2","G2");
	IModelFunction f3 = (IModelFunction) ff.createFunctionByName("f3","g2");
	IModelFunction f4 = (IModelFunction) ff.createFunctionByName("f4","G2");
        ...
        hep.aida.IHistogram2D hist2d = hf.createHistogram2D("hist2d","hist2d",50,-10,10,50,-10,10);
        java.util.Random r = new java.util.Random();
        
        ITuple tup = tf.create("tuple-2D","tuple with 2D Gaussian","double x0, double x1");
        
	
        for ( int i = 0; i < 50000; i++ ) {
            double x = 2.*(r.nextGaussian())-2.;
            double y = 3.*(r.nextGaussian())+2.;
	    hist2d.fill(x,y);
	    tup.fill(0,x);
	    tup.fill(1,y);
	    tup.addRow();
	...
	//f1.normalize(false);
	//f2.normalize(false);
	double[] chi2 = new double[] { 100., -1., 5., 0., 5. };
	f1.setParameters(chi2);
	f2.setParameters(chi2);

        Fitter fitter = new Fitter("Chi2","minuit",null);
        //Fitter fitter = new Fitter("Chi2","uncmin");
        IFitResult fr = fitter.fit(hist2d, f2);
        //fitter.fit(hist1,"G");

	((FitResult) fr).printResult();

	plotter.currentRegion().plot(hist2d);
