...
public abstract class AbstractTuple extends ManagedObject implements ITuple, FTuple {
    ...
    public Object columnDefaultValue(int column) {
        ...
        if ( type == Integer.TYPE )
            return new Integer(columnDefaultString(column));
        else if ( type == Short.TYPE )
            return new Short(columnDefaultString(column));
        else if ( type == Long.TYPE )
            return new Long(columnDefaultString(column));
        else if ( type == Float.TYPE )
            return new Float(columnDefaultString(column));
        else if ( type == Double.TYPE )
            return new Double(columnDefaultString(column));
        else if ( type == Boolean.TYPE )
            return new Boolean(columnDefaultString(column));
        else if ( type == Byte.TYPE )
            return new Byte(columnDefaultString(column));
        else if ( type == Character.TYPE )
            return new Character(columnDefaultString(column).toCharArray()[0]);
        else if ( type == ITuple.class )
            return new Tuple(name(), title(), columnDefaultString(column), null);
        else
            return columnDefaultString(column);
    }
