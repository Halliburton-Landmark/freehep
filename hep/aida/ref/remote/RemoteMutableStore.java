...
public abstract class RemoteMutableStore implements AidaUpdatable, IDevMutableStore {
    ...
    public void reloadFolder(String path)  throws IllegalArgumentException, IOException {
        remoteLogger.fine("RemoteMutableStore.reloadFolder: path="+path+", recursive="+isRecursive());
        if (!client.isConnected())
            throw new IOException("Can not read from the Store: client is not connected!");

        Tree localTree = (Tree) tree;
        
        String[] names = client.listObjectNames(path, isRecursive());
        String[] types = client.listObjectTypes(path, isRecursive());
        String[] oldNames = localTree.listObjectNames(path, isRecursive());
        String[] oldTypes = localTree.listObjectTypes(path, isRecursive());
        ...
        if (names != null && types != null) {
            ...
            for (int i=0; i<names.length; i++) {
                ...
                try {
                    ...
                    remoteLogger.log(Level.FINE, "Exception in RemoteMutableStore.reloadFolder: "+ e.getMessage());
                    remoteLogger.log(Level.FINEST, "Exception in RemoteMutableStore.reloadFolder: ", e);
                }
            }
    ...
    public void read(IDevTree tree, String path) throws IllegalArgumentException, IOException {
        ...
        String[] names = null;
        String[] types = null;
        names = client.listObjectNames(path, isRecursive());
        types = client.listObjectTypes(path, isRecursive());
        ...
        if (names != null && types != null) {
            for (int i=0; i<names.length; i++) {
                try {
                    ...
                    remoteLogger.log(Level.FINE, "Exception in RemoteMutableStore.read: "+ e.getMessage());
                    remoteLogger.log(Level.FINEST, "Exception in RemoteMutableStore.read: ", e);
                }
            }
    ...
    protected void executeStateChanged(int id, String path, String type, String xType) {
        ...
        if (id == AidaUpdateEvent.NODE_ADDED || id == AidaUpdateEvent.DO_ADD_NODE_NOW) {
            try {
                if (type.equalsIgnoreCase("dir") || type.equalsIgnoreCase("mnt")) {
                    ...
                    try {
                        ...
                        remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: Exception while adding node: "+type+", "+path+", Skip this.", e);                     
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while ADDING node. Skip this: \n\t"+ex.getMessage());
            ...
            try {
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while DELETING node. Skip this.", ex);
            }
            ...
            try {
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while UPDATING node. Skip this.", ex);
            }
        } else if (id == AidaUpdateEvent.DO_DATA_UPDATE_NOW) {
            updateData(path, type);
        } else if (id == AidaUpdateEvent.TREE_CLOSED) {
            remoteLogger.info("RemoteMutableStore.executeStateChanged: Got TREE_CLOSED event from Remote AIDA Server. Exiting");
            acceptEvents = false;
            ...
            try {
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while NODE_TEMPORARY_UNAVAILABLE operation. Skip this.", ex);
            }
            ...
            try {
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while NODE_IS_AVAILABLE_AGAIN operation. Skip this.", ex);
            }
            ...
            try {
                if (tree instanceof RemoteTree) {
                    if (type.equalsIgnoreCase("dir")) {
                        ...
                        if (isRecursive()) try {
                            ...
                            remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: Exception while updating PATH="+path, ioe);
                ...
                remoteLogger.log(Level.INFO, "RemoteMutableStore.executeStateChanged: IllegalArgumentException while NODE_IS_AVAILABLE_AGAIN operation. Skip this.", e);
            }
        } else {
