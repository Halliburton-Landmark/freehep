 ...
 *
 * @author tonyj
 * @version $Id: TestRHistogram.java,v 1.5 2005/06/15 04:20:00 chuli Exp $
 */
public class TestRHistogram extends RAIDATestCase {
    ...
    public void testCaseRHistogram() {
		...
		IHistogram1D h12 = null;
		try{
			h12 = hf.createCopy(histNewPath + "/" + hist1DTitle, h1);
		...
		/* IHistogram1D rh12 = null;
		try {
            rh12 = (IHistogram1D) clientTree.find(histNewPath + "/" + hist1DTitle);
		} catch (Exception e) {
			System.out.println("find exception caught on client tree!");
			e.printStackTrace();
		}
		assertTrue(rh12 != null); */
		
		//-------------------------------------------------------
		//scale the IHistogram1D instance;
		myscaler = r.nextDouble();
		System.out.println("Scale the Histogram1D instance on server tree with scaler: " + myscaler);
		h1.scale(myscaler);
		TestRUtils.waitForAWhile(timeout);
		...
		*/
		...
		myscaler = r.nextDouble();
		System.out.println("Scale the Histogram2D instance on server tree with scaler: " + myscaler);
		h2.scale(myscaler);
		TestRUtils.waitForAWhile(timeout);
		//The update of h1 on server side should be reflected on client tree (DuplexMode);
		assertEquals(h2, (RemoteHistogram2D)rh2);
