 ...
 *
 * @author tonyj
 * @version $Id: TestRCloud.java,v 1.2 2005/06/06 16:50:46 chuli Exp $
 */
public class TestRCloud extends RAIDATestCase {
    ...
    public void testCaseRCloud() {
		...
		System.out.println("Scale the ICloud1D instance on server tree with scaler: " + myscaler);
		c1.scale(myscaler);
		TestRUtils.waitForAWhile(timeout);
		...
		myscaler = r.nextDouble();
		System.out.println("Scale the ICloud2D instance on server tree with scaler: " + myscaler);
		c2.scale(myscaler);
		TestRUtils.waitForAWhile(timeout);
